...
void core_layer::handle_interest(ccn_interest *int_msg)
{
   ...
   chunk_t chunk = int_msg->getChunk();
    ...
    long f=0;
    __sface(f, int_msg->getArrivalGate()->getIndex());

    if (ContentStore->lookup(chunk, 1, f)){
        //
        //a) Check in your Content Store
        //
        ...
        //std::cout << b << endl;
        //std::cout << int_msg->getArrivalGate()->getIndex() << endl;
...
ccn_data* core_layer::compose_data(uint64_t response_data){
    ccn_data* data = new ccn_data(("data_" + std::to_string(response_data)).c_str(),CCN_D);
